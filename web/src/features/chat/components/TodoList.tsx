...
export function TodoList({ items = DEFAULT_LABELS, startDelay = DEFAULT_START_DELAY, stepMs = DEFAULT_STEP_MS }: { items?: string[]; startDelay?: number; stepMs?: number }) {
  ...
  useEffect(() => {
    ...
    const timers = [setTimeout(() => setCurrent(0), startDelay)];
    for (let i = 0; i < n; i++) {
      timers.push(setTimeout(() => setCurrent(i + 1), startDelay + (i + 1) * stepMs));
    }
