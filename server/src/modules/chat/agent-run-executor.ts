...
export class AgentRunExecutor {
  ...
  async execute(runId: string, signal: AbortSignal, resumed: boolean): Promise<'completed' | 'waiting_approval'> {
    ...
    const session = new PrismaAgentSession(sessionId, loadSessionCompactionOptions(), {
      started: async ({ id, trigger }) => {
        await this.emit(conversation.id, runId, 'context_compaction.started', { id, trigger })
      },
      ...
      failed: async ({ id, trigger, error }) => {
        await this.emit(conversation.id, runId, 'context_compaction.failed', { id, trigger, error })
      },
    })

    const input = await this.loadInput(agent, agentRun.input, agentRun.state)
    const state = input instanceof RunState ? await this.applyApprovals(agentRun.id, conversation.id, input) : undefined
    await this.emit(conversation.id, runId, resumed ? 'run.resumed' : 'run.started', {})
    ...
    await this.consumeStream(stream, conversation.id, runId, streamState)

    const interruptions = stream.interruptions
    if (interruptions.length > 0) {
      await prisma.agentRun.update({
        where: { id: runId },
        data: { status: 'waiting_approval', state: stream.state.toString(), lastHeartbeatAt: new Date() },
      })
      await this.emit(conversation.id, runId, 'run.waiting_approval', {})
      for (const interruption of interruptions) {
        ...
        await this.emit(conversation.id, runId, 'tool.awaiting_approval', {
          toolCallId: details.toolCallId,
    ...
    await this.completeRun(agentRun.id, conversation.id, runId, stream, session)
    return 'completed'
  }
  ...
  private async applyApprovals(
    ...
    for (const interruption of interruptions) {
      ...
      if (approval.status === 'approved') {
        ...
        await this.emit(conversationId, runId, 'tool.approved', { toolCallId: callId })
      } else {
        state.reject(interruption)
        await this.emit(conversationId, runId, 'tool.denied', { toolCallId: callId })
      }
    }
    await prisma.agentRun.update({ where: { id: runId }, data: { state: state.toString() } })
    return state
  }
  ...
  private async consumeStream(
    ...
    for await (const event of stream as AsyncIterable<RunStreamEvent>) {
      await persistRunStreamEvent(this.timelineStore, conversationId, runId, event, streamState)
  ...
  private async completeRun(
    ...
    await this.emit(conversationId, timelineRunId, 'run.completed', { output: finalOutput })
  }
